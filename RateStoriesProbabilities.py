...
One fateful day, as the sun set on the eastern horizon of our planet, an anomaly occurred. The radio telescopes at SETI (Search for Extraterrestrial Intelligence) Institute picked up a signal. It was unlike anything they had ever encountered before. The signal, pulsating at regular intervals, was not random but seemed to28781 to make sense of it. Days turned into weeks, and weeks into months. The signal was not a noise; it was a message.
...
for key, entry in tqdm.tqdm(stories.items()):
    ...
    for crit, prompt in prompts.items():
        ...
        print(logits.argmax())
        print(f'token: {tokenizer.decode_unspecial(int(logits.argmax()))}')
        print()
