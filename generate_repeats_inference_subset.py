...
model.load_autosplit(cache, reserve_vram=[6144*1024**2, 1024*1024**2])
...
def saveData(modelOutput, layersDict, prompt, settings, max_new_tokens, seed, theme, modelName, number):
    experiment = {}
...
for i, (key, layers) in tqdm(enumerate(layersDict.items()), total=len(layersDict)):
    ...
    if key in modelOutput:
        print(f"{key=} already in modelOutput")
        continue
    ...
    print()
    if i % 20 == 0:
        saveData(modelOutput, layersDict, prompt, settings, max_new_tokens, seed, theme, modelName, number)
